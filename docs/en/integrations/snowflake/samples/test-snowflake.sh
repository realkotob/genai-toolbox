...
echo "Command: ./toolbox --tools-file docs/en/integrations/snowflake/samples/snowflake-config.yaml --stdio"
echo ""
echo "Expected result: Connection failure due to test credentials (this is normal)"
echo ""

# Test the custom configuration (this will fail with test credentials, which is expected)
timeout 5s ./toolbox --tools-file docs/en/integrations/snowflake/samples/snowflake-config.yaml --stdio 2>&1 | head -5
